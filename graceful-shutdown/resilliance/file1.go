...
func spawnNewInstance(executable string) (*os.Process, error) {
	fmt.Println("Spawning a new instance of file1.go...")
...
func main() {
	pid := os.Getpid()
	fmt.Printf("running file1.go with PID[%d] ...\n", pid)
	...
	fmt.Println("Waiting to read PID from file2.go ...")
	...
	for {
		...
		if err != nil {
			...
			fmt.Printf("file2.go is running with PID[%d]\n", sharedPID)
			fmt.Printf("Forwarding SIGINT to file2.go (PID: %d)...\n", sharedPID)
			forwardSIGINT(sharedPID)
		}

		<-sigCh

		fmt.Printf("Received SIGINT signal from file2.go with PID[%d]. Initiating restart...\n", sharedPID)
